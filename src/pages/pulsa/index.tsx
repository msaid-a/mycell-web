  ...
  const changeOperator = (e: any) => {
    if (dataPaket) {
      const result = dataPaket?.filter((elm) => {
        ...
        return elm.opertator === e.target.value && elm.type === "pulsa";
      });
      if (result.length > 0) {
        setData(result);
    ...
    <div>
      ...
      <Formik
        ...
        onSubmit={(values, actions) => {
          window.open(
            `https://api.whatsapp.com/send/?phone=6281218389762&text=No%20Handpone:%20${values.phone_number}%0ANama:%20${values.name}%0ANomimal%20Pulsa:%20${values.nominal}%0AHarga:%20${totalPrice}&type=phone_number&app_absent=0`
          );
        }}
      >
        {(props) => {
          return (
            <Form onSubmit={props.handleSubmit}>
              ...
              >
                <FormLabel>Pilih Operator</FormLabel>
                <Field
                  as={Select}
